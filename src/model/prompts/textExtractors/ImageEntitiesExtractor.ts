...
- name
- emoji
- traits (up to 5 short visual traits, e.g. old, tall, armored, red-haired)

Rules:
- Return only distinct entities (merge duplicate mentions).
- Exclude actions, emotions, intentions, personality, relationships, and locations.
  ...
  static normalize(entities: z.infer<typeof IMAGE_ENTITIES_SCHEMA>["entities"]): ExtractedImageEntity[] {
    ...
    for (const entity of entities) {
      ...
      const traits = Array.isArray(entity.traits)
        ? entity.traits.map((t) => String(t).trim()).filter((t) => t.length > 0).slice(0, 5)
        : [];
