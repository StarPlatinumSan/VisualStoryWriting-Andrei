...
interface ModelAction {
	...
	setAiProvider: (provider: "openai" | "local") => void;
...
function getInitialState() {
	...
	const initialState: ModelState = {
		...
		highlightedEntities: [],
		textActionMatches: TextUtils.matchActionsToText(
			actionEdges.map((edge) => edge.data!),
			text,
		),
...
export const useModelStore = create<ModelState & ModelAction>()((set, get) => ({
	...
	getFilteredEntityNodes: (actionFilter: { start: number; end: number } | null) => {
		...
		const filteredIds = get()
			.getFilteredActionEdges(actionFilter)
	...
	getFilteredLocationNodes: (actionFilter: { start: number; end: number } | null) => {
		// Filter to only keep the nodes that are not filtered out
		const filteredLocations = get()
			.getFilteredActionEdges(actionFilter)
			.map((edge) => [edge.data!.sourceLocation, edge.data!.targetLocation])
			.flat();
	...
	setActionEdges: (edges) => {
		...
		const textActionMatches = TextUtils.matchActionsToText(
	...
	setTextState: (textState, updateEditor = true, addHistoryNode = true) => {
		...
		if (isTextDifferent) {
			...
			const textActionMatches = TextUtils.matchActionsToText(
				get().actionEdges.map((edge) => edge.data!),
				text,
			);
		...
		if (updateEditor) {
			// Prevent stale Slate selections from crashing on route/text switches.
	...
	getText: () => {
		return get()
			.textState.map((node: any) => SlateNode.string({ children: node.children.filter((c: any) => c.removed === undefined) }))
			.join("\n");
	},
