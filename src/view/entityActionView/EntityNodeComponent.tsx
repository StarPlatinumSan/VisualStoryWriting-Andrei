...
export function CreateEntityNode(entity: Entity, index: number): EntityNode {
	...
	const rawEmoji = getEntityEmoji(entity.emoji);
...
export default function EntityNodeComponent(props: NodeProps<EntityNode>) {
	...
	if (!isFaded && highlightedEntities.length > 0 && highlightedEntities.indexOf(props.id) === -1) {
		isFaded = useModelStore.getState().actionEdges.filter((edge) => (highlightedEntities.includes(edge.source) && edge.target === props.id) || (highlightedEntities.includes(edge.target) && edge.source === props.id)).length === 0;
	}
	...
	const onPropertySliderChanged = useCallback(
		(property: string, newValue: number, triggerPrompt: boolean = false) => {
		...
		[entityNodes, setEntityNodes, props.id],
	);

	const propertySliders = props.data.properties.map((property) => {
		return (
			<div className="nodrag nopan" style={{ display: "flex", flexDirection: "column" }} key={`property-${props.data.name}-${property.name}`}>
				<Slider size="sm" label={property.name} className="max-w-md" step={1} color="primary" minValue={1} maxValue={10} defaultValue={property.value} onChangeEnd={(newValue) => onPropertySliderChanged(property.name, newValue as number, true)} />
			</div>
		);
	});

	const rawEmoji = getEntityEmoji(props.data.emoji);
	...
	return (
		<>
			<div
				className="custom-drag-handle node-entity"
				style={{
					position: "relative",
					border: `1px solid ${isSelected ? "#4180d9" : "white"}`,
