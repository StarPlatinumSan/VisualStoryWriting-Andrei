...
export default function ImagePaintEditor(props: {
  ...
  const buildSavePayload = (requireDrawMarks = false, requirePromptText = false, requirePromptOrDraw = false): ImagePaintSavePayload | null => {
    ...
    for (let i = 0; i < drawImageData.data.length; i += 4) {
      ...
      if (requirePromptOrDraw && !hasDrawMarks && trimmedPrompt.length === 0) {
        setError("Ajoute un texte d'edition ou dessine sur l'image avant de regenerer.");
        return null;
      }
      if (requireDrawMarks && !hasDrawMarks) {
        setError("Dessine d'abord sur l'image avant de regenerer.");
        return null;
      }
      if (requirePromptText && trimmedPrompt.length === 0) {
        setError("Le texte d'edition est obligatoire avant de regenerer.");
        return null;
      }
  ...
  const saveMergedImage = () => {
    ...
    const saveAndApply = () => {
      ...
      const payload = buildSavePayload(false, false, true);
      if (!payload) return;
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", border: "1px solid #e5e7eb", background: "white", borderRadius: 10, padding: 10 }}>
        ...
        <button
          ...
          onClick={undo}
          disabled={!hasUndo}
          style={{ border: "1px solid #d1d5db", borderRadius: 8, padding: "6px 10px", background: hasUndo ? "white" : "#f3f4f6", cursor: hasUndo ? "pointer" : "not-allowed" }}
        >
          Undo
        </button>
        <button
          ...
          onClick={redo}
          disabled={!hasRedo}
          style={{ border: "1px solid #d1d5db", borderRadius: 8, padding: "6px 10px", background: hasRedo ? "white" : "#f3f4f6", cursor: hasRedo ? "pointer" : "not-allowed" }}
        >
          Redo
        </button>
      ...
      <div style={{ border: "1px solid #e5e7eb", background: "white", borderRadius: 10, padding: 10 }}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 6 }}>Prompt d'edition (optionnel)</div>
        <textarea
